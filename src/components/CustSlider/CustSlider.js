...
class CustSlider extends React.Component {
    ...
    sliderWechsel = (value) => {
        if (typeof this.props.onChange === 'function') {
            this.props.onChange({untereGrenze: value[0]*7, obereGrenze: value[1]*7});
        }
        this.setState({
            value,
        });
    ...
    render() {
        return (
            <div style={style}>
                <Range allowCross={false} value={this.state.value} onChange={this.sliderWechsel}  tipFormatter={value => `${value*7}€`} />
            </div>
        );
    }
}
